...
namespace WebApi8_Scheduling.Services.Client
{
    public class ClientService : IClientInterface
    {
        ...
        public async Task<ResponseModel<ClientModel>> CreateClient(ClientCreateDto pClient)
        {
            ...
            try
            {
                var EnterpriseId = await _context.Enterprise.FirstOrDefaultAsync(a => a.Id == pClient.EnterpriseId);
                ...
                var newClient = new ClientModel()
                {
                    Name = pClient.Name,
                    Tel = pClient.Tel,
                    EnterpriseId = pClient.EnterpriseId
                };
        ...
        public async Task<ResponseModel<ClientModel>> UpdateClient(int clientId, ClientUpdateDto clientUpdateDto)
        {
            ResponseModel<ClientModel> respost = new ResponseModel<ClientModel>();

            try
            {
                var user = await _context.Clients.FirstOrDefaultAsync(a => a.Id == clientId);
